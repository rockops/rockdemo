...
| `mount` | Optional list of bind mount objects `[{"src": "/path/source", "dest": "/path/dest"}]` to mount host directories into this node's container. Accepts optional `"ro": true` or `"readOnly": true` for read-only mounts. If `src` does not exist on the host, the mount is skipped. |
